    ...
    refSolThExp = GetConstReal( material, 'Reference Thermal Expansivity of Solid Ti ',GotIt)
    IF(.NOT. GotIt) THEN
    CALL Fatal('getThermalStressCoefficient', 'Reference Thermal Expansivity of Solid Ti  not found')
    END IF

    ! read in Temperature Coefficient of Resistance
    alphas = GetConstReal( material, 'Exp Coeff Solid TiAlV', GotIt)
    IF(.NOT. GotIt) THEN
    CALL Fatal('getThermalStressCoefficient', 'slope of thermal expansivity-temperature curve solid not found')
    END IF
    
    ! read in reference conductivity at reference temperature
    refLiqThExp = GetConstReal( material, 'Reference Thermal Expansivity of Liquid Ti ',GotIt)
    IF(.NOT. GotIt) THEN
    CALL Fatal('getThermalStressCoefficient', 'Reference Thermal Expansivity of Liquid Ti  not found')
    END IF
    
    ! read in Temperature Coefficient of Resistance
    alphal = GetConstReal( material, 'Exp Coeff Liquid Ti ', GotIt)
    IF(.NOT. GotIt) THEN
    CALL Fatal('getThermalStressCoefficient', 'slope of thermal expansivity-temperature curve Liquid Ti  not found')
    END IF
