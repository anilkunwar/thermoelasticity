    ...
    refSolThExp = GetConstReal( material, 'Reference Thermal Expansivity of Solid Ti3Au',GotIt)
    IF(.NOT. GotIt) THEN
    CALL Fatal('getThermalStressCoefficient', 'Reference Thermal Expansivity of Solid Ti3Au not found')
    END IF
    ...
    ! read in reference conductivity at reference temperature
    refLiqThExp = GetConstReal( material, 'Reference Thermal Expansivity of Liquid Ti3Au',GotIt)
    IF(.NOT. GotIt) THEN
    CALL Fatal('getThermalStressCoefficient', 'Reference Thermal Expansivity of Liquid Ti3Au not found')
    END IF
    ...
    refTemp = GetConstReal( material, 'Melting Point Temperature Ti3Au', GotIt)
    IF(.NOT. GotIt) THEN
