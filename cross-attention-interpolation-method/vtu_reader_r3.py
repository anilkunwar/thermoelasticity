...
if st.button("🔄 Convert to Temporal .pt Sequence", type="primary"):
    with st.spinner(f"Reading {sim['files']} timesteps with temporal sequencing..."):
        ...
        cumulative_times = np.cumsum([0] + sim["time_pattern"])[:-1]  # Start at t=0
        
        all_data = []
        progress_bar = st.progress(0)
        
        for i, (vtu_path, time_val) in enumerate(tqdm(zip(vtu_files, cumulative_times), 
...
if saved:
    ...
    if st.button("Load Temporal Sequence"):
        conn = sqlite3.connect(SCRIPT_DIR / "laser_temporal.db", check_same_thread=False)
        row = conn.execute(
            "SELECT pt_data FROM temporal_sims WHERE name = ?", (load_name,)
        ).fetchone()
        conn.close()
        ...
        if row:
            ...
            # Time slider
            t_idx = st.slider("Time step", 0, data['metadata']['n_timesteps']-1, 0)
